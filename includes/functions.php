<?php
/**
 * Core utility functions — compatible MySQL et PostgreSQL
 */
...
function generateMatricule(int $anneeId): string {
    ...
    if (dbIsMySQL()) {
        $db->prepare("UPDATE sequences SET value = LAST_INSERT_ID(value + 1) WHERE name_seq = 'matricule'")->execute([]);
        $num = (int)$db->query("SELECT LAST_INSERT_ID()")->fetchColumn();
    } else {
        $num = (int)$db->query("SELECT nextval('seq_matricule')")->fetchColumn();
    }
...
function generateNumeroRecu(): int {
    $db = getDB();
    if (dbIsMySQL()) {
        $db->prepare("UPDATE sequences SET value = LAST_INSERT_ID(value + 1) WHERE name_seq = 'numero_recu'")->execute([]);
...
function getSituationFinanciere(int $eleveId): array {
    ...
    // Compatible MySQL et PostgreSQL : CASE WHEN remplace FILTER (WHERE ...)
    $tranchesStmt = $db->prepare("
        SELECT t.id, t.grille_id, t.numero, t.libelle_fr, t.libelle_en, t.montant, t.echeance_indicative,
               COALESCE(SUM(CASE WHEN p.annule = FALSE THEN p.montant ELSE 0 END), 0) AS paye
        FROM tranches t
        LEFT JOIN paiements p ON p.tranche_id = t.id AND p.eleve_id = ?
        WHERE t.grille_id = ?
        GROUP BY t.id, t.grille_id, t.numero, t.libelle_fr, t.libelle_en, t.montant, t.echeance_indicative
        ORDER BY t.numero
    ");
    ...
    $tauxReduction    = min(($reductionFratrie + $reductionComplet), 20);
